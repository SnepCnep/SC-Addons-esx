...
RegisterCommand(Config.takehostagecmd,function()
	callTakeHostage()
end)

RegisterCommand(Config.takehostage2cmd,function()
	callTakeHostage()
end)
...
function callTakeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification(Config.noammo)
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				drawNativeNotification(Config.erisniemand)
